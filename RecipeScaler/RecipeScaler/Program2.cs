...
namespace RecipeScaler
{
    class Program2
    {
        ...
        {
            RecipeManager recipeManager = new RecipeManager();  // Create a new instance of RecipeManager
            ...
            while (true)
            {
                ...
                int option = int.Parse(Console.ReadLine());  // Read the user's option

                switch (option)
                {
                    case 1:
                        AddRecipe(recipeManager);  // Call the AddRecipe method
                        break;
                    case 2:
                        DisplayAllRecipes(recipeManager);  // Call the DisplayAllRecipes method
                        break;
                    case 3:
                        ChooseRecipeToDisplay(recipeManager);  // Call the ChooseRecipeToDisplay method
                        break;
                    case 4:
        ...
        static void AddRecipe(RecipeManager recipeManager)
        {
            ...
            Recipe recipe = new Recipe(name);  // Create a new instance of Recipe
            ...
            for (int i = 0; i < numIngredients; i++)
            {
                ...
                Ingredient ingredient = new Ingredient(ingredientName, quantity, unit, calories, foodGroup);  // Create a new instance of Ingredient
            ...
            for (int i = 0; i < numSteps; i++)
            {
                ...
                recipe.AddStep(description);  // Add the step to the recipe
            }

            recipeManager.AddRecipe(recipe);  // Add the recipe to the recipe manager
        ...
        // Method to choose and display a recipe
        static void ChooseRecipeToDisplay(RecipeManager recipeManager)
        {
            ...
            if (choice >= 1 && choice <= recipes.Count)
            {
                ...
                selectedRecipe.PrintRecipe();  // Print the selected recipe
